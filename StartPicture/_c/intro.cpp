...
void exportLogo(const char *name)
{
	...
	int frontColor = 0x71;//pictureS[0];
...
void saveText()
{
	...
	char *remapArray = "ABCDEFGHIJKLMNOPQRSTUVWXYZ 1234567890!?':,;.ö()-*öööä";
	char *colortable = "8923b923892389238923892389138923892389238923892389238";
...
void buildFadeOutStuff()
{
	...
	for (int i = 0; i < 256; ++i)
	{
		int switch1 = rand() % 256;
		int switch2 = rand() % 256;
	...
	out = fopen((globalOutputDirPrefix+"specialtable.bin").c_str(),"wb");
	unsigned char c[256];
	for (int i = 0; i < 256; ++i) c[i] = i % 100;
...
void exportLoadingPicture(const char *name)
{
	...
	int frontColor = 0x48; //pictureS[0];
	int backColor = 0x12; //pictureS[2];
...
void intro_init( void )
{
	...
	pictureHeight = 64;
	buildFont();
//	exportLogo((globalWorkingDirPrefix + "zad5.png").c_str());
	saveText();
	exportTables();
	exportdotaddtable();
	exportMask();
	buildFadeOutStuff();
//	exportLoadingPicture((globalWorkingDirPrefix + "zador_helge.png").c_str());
	exportLoadingPicture((globalWorkingDirPrefix + "zador_preview.png").c_str());
}
